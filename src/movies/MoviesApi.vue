<template>
    ...
    <div v-else class="container text-center mt-4">
        <h2 class="text-light text-bg-secondary font-bold mt-4 mb-4 p-3 rounded-4" style="letter-spacing: 10px;">Popular
            Movies
        </h2>
        <div v-for="m in data" :key="m.id">
            <RouterLink :to="`/movie/${m.id}`" class="btn btn-primary fw-bold mt-4 w-25">{{ m.title }}</RouterLink>
        </div>
    </div>
</template>
